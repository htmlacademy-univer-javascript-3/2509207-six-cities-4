...
export default function ReviewForm({ id }: ReviewFormProps): JSX.Element {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (review.rating > 0 && review.text.length >= 50) {
      dispatch(addReview({ id, comment: review.text, rating: review.rating }));
    }
  };
  ...
  return (
    <form className="reviews__form form" action="#" method="post" onSubmit={handleSubmit}>
      ...
      <textarea className="reviews__textarea form__textarea" id="review" name="review" placeholder="Tell how was your stay, what you like and what can be improved" onChange={handleTextChange}>
      </textarea>
      <div className="reviews__button-wrapper">
        <p className="reviews__help">
          To submit review please make sure to set <span className="reviews__star">rating</span> and describe your stay with at least <b className="reviews__text-amount">50 characters</b>.
        </p>
        <button className="reviews__submit form__submit button" type="submit" disabled={review.rating === 0 || review.text.length < 50}>
          Submit
        </button>
      </div>
    </form>
